...
fix=1
nosummary=0
# Set default branch from environment. Else, if in LIVE mode, fall back to master branch. otherwise fallback to master branch
...
if [ $showhelp = 1 ]; then
    ...
    echo ""
    echo "usage: $0 <branch> [--help] [--force | -f] [--no-migrate | -n] [--kill-modules | -ff ] [--no-compile] [--no-pull] [-r <remote>] [--no-summary] [--master | -d] [-u<username>]  [-p<password>]"
    echo ""
    ...
    echo "  --master "
    echo "           |-d   : If specified branch is not found, fallback to master branch"
